    ...
    <>
          ...
          {/* <Stage width={4000} height={2000}> */}
            ...
            {shapes.map((shape, i) => {
              switch (shape.type) {
                case 'rect':
                  return <Rect key={i} x={shape.x} y={shape.y} width={shape.width} height={shape.height} fill={shape.fill} draggable />;
                case 'circle':
                  return <Circle key={i} x={shape.x} y={shape.y} radius={shape.radius} fill="green" draggable />;
                case 'triangle':
